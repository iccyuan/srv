...
func TestTunnelListenLabel(t *testing.T) {
	prof := &Profile{Host: "db.internal"}
	cases := []struct {
		def  *TunnelDef
		want string
	}{
		{&TunnelDef{Type: "local", Spec: "5432"}, "127.0.0.1:5432"},
		{&TunnelDef{Type: "local", Spec: "5432:db:5432"}, "127.0.0.1:5432"},
		{&TunnelDef{Type: "remote", Spec: "9000:3000"}, "db.internal:9000"},
	}
	for _, c := range cases {
