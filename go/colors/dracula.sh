...
#
# This file is special among the built-ins: it FIRST tries the user's
# own dircolors database (~/.dir_colors / ~/.dircolors) so a hand-
# tuned palette wins over our default. Other shipped themes (gruvbox,
# nord, etc.) override unconditionally because the user explicitly
# picked them via `srv color use <name>`.

if [ -z "$LS_COLORS" ]; then
    if [ -r "$HOME/.dir_colors" ]; then
        eval "$(dircolors -b "$HOME/.dir_colors" 2>/dev/null)" 2>/dev/null
    elif [ -r "$HOME/.dircolors" ]; then
        eval "$(dircolors -b "$HOME/.dircolors" 2>/dev/null)" 2>/dev/null
    fi
fi
