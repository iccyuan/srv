...
> 起步版本 **2.0.0**。 用 `golang.org/x/crypto/ssh` 自实现 SSH 客户端,**不再依赖系统 ssh.exe**。Python 版本仍保留在 [`../src/`](../src),功能完全对等。
...
# 注册(Windows 路径示例)
claude mcp add srv --scope user -- D:\WorkSpace\server\srv\go\srv.exe mcp
