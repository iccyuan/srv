...
- [x] sync gzip 压缩 `compress_sync`(2.4.1)+ `sync --watch`(2.x)+ `sync --delete` git 模式和删除保护(2.6)
- [x] **`tunnel`** —— ssh -L / -R 等价的 TCP 转发(2.5/2.7)
- [x] **`edit`** —— SFTP 拉到 temp / `$EDITOR` / mtime 改了且远端未变更才推回(2.5/2.7)
- [x] **`open` / `code` / `diff` / `diff --changed` / `doctor --json`** —— 本地工作流辅助(2.6)
