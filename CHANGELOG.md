...
- **guard 新增全局开关 `srv guard off|on --global`**(修复 default-on 在 Unix 下的盲区):per-session guard 按 ppid 取 session id,MCP server 是 Claude Code 子进程,session 与交互 shell 不同 —— 默认开之后,终端里 `srv guard off` **关不掉 MCP server 的 guard**。新增机器级开关 `config.GuardConfig.GlobalOff *bool`,写入 `config.json`(MCP server 每次调用实时重读,无需重启)。优先级:`SRV_GUARD` env > 当前 shell session > **global config** > 内置默认开。`session.GuardOn()` 拆出三态 `session.GuardPref()`(env+session,无默认),全局/默认层下沉到新的 `config.GuardActive()`(config 已 import session,无循环;反向不行)。所有 guard 消费点(`guardCheckRisky`、sync `--delete` gate、`status`/meta 上报)改走 `config.GuardActive()`。`srv guard status` 现在显示生效值 + 来源层。
- **guard 内置集补全跨平台 / NoSQL 盲区**:① 关系库 `DROP` 增加 Cassandra `KEYSPACE`;② NoSQL —— MongoDB `dropDatabase()`/`db.<coll>.drop()`、Redis `FLUSHALL`/`FLUSHDB`、PostgreSQL CLI `dropdb`;③ macOS —— `> /dev/disk` 正则扩成 `r?disk`(裸盘 `/dev/rdisk0`)、新增 `newfs_*`(macOS 无 `mkfs`)和 `diskutil erase*/partitionDisk/zeroDisk/secureErase/apfs delete*`(`diskutil list/info/mount` 不拦)。已加 FP 防护:pandas `df.drop(`、lodash `_.drop(`、`createdb`、`>/dev/null` 等不误判。
- **guard 拦截引号内 DB payload**(新增 `sql -e drop` / `mongo --eval drop` 规则):`mysql -e "DROP DATABASE x"`、`psql -c "..."`、`mysql --execute=`、`cqlsh -e`、`mongosh --eval "db.dropDatabase()"`/`db.x.drop()` 现在会拦。做法:把匹配锚在**未加引号的客户端二进制**(`mysql`/`psql`/`mongosh`…)上而非引号里的 verb —— 该字节在 code position,所以 `echo "mysql -e \"DROP DATABASE x\""` 这种整体被 echo 包住的仍**不**误杀(客户端名自身落在引号内,codePositions 抑制)。客户端→flag、flag→verb 两段都用 `[^|;&\n]`,verb 必须和客户端在**同一条简单命令**里,`psql -c "SELECT 1" && echo "...drop database..."` 不会触发。RE2 有界量词,线性。**残留局限**(可接受、可 confirm 绕过):仅 DB 客户端的 `-e/--eval/-c` 形式被覆盖;`bash -c "mysql ...«heredoc»"`、把 SQL 写进文件再 `mysql < f.sql` 这类间接形式仍看不到。
